...
def get_args():
    ...
    parser.add_argument('--fine', type=str, default='CIFAR10', help='File to save model.')
    parser.add_argument('--model_file', type=str, default='model.pth.tar', help='File to save model.')
    parser.add_argument('--model_dir',  type=str, default='save_models', help='Directory for storing checkpoint file.')
    ...
    parser.add_argument('--distr_type',   type=str,   default='dirichlet', choices=('uniform', 'dirichlet'), help='Distribution to construct local data.')
    ...
    parser.add_argument('--num_locals',  type=int,   default=5, help='number of local agents.')
    parser.add_argument('--num_per_rnd', type=int,   default=5, help='number of local agents to train per round.')
    parser.add_argument('--rounds',      type=int,   default=300, help='number of communication rounds.')
    parser.add_argument('--sample_rate', type=float, default=-1., help='sample rate of central data.')
    parser.add_argument('--policy',      type=str,   default='ewc', choices=('avg', 'ewc', 'mas'), help='Policy for estimating parameter importance.')
    parser.add_argument('--estimate_weights_in_center',default=False, action='store_true', help='Estimate parameter importance in central server.')

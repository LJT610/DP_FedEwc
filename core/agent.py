...
class Agent(ABC):
    ...
    def ewc(self, train_loader=None):
        ...
        self.model.eval()
        num_examples = 0 #此处用的完整的batchsize
        for image_batch, label_batch in BackgroundGenerator(train_loader):
            ...
            loss.backward()
            for k, p in self.model.named_parameters():
                tmp_weights[k].add_(p.grad.detach() ** 2) #PI

        for k, v in tmp_weights.items():
            # tmp_weights[k] = torch.sum(v).div(num_examples).cpu()
            tmp_weights[k] =  v.cpu()/num_examples
    ...
    def train(self, a,b):
        ...
        for image_batch, label_batch in BackgroundGenerator(self.train_loader):
            ...
            #         cons_loss += w_d[k] * torch.sum((v - self.global_model[k]) ** 2)
            #         # cons_loss += w_d[k] * torch.sum(v ** 2)
            #     cons_loss *= self.coe
            #     cons_loss_meter.update(cons_loss.item(), label_batch.size(0))

            if (a is not None) and (b is not None):
                for k,v in self.model.named_parameters():
                    cons_loss += torch.sum(a[k] * (v ** 2)) - 2 * torch.sum(v * b[k]) #sum represent the sum step of matrix mul
                cons_loss *= self.coe
                cons_loss_meter.update(cons_loss.item(), label_batch.size(0))
